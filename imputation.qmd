...
coef 0.5 0.5
#
# TODO consider by county... except some even morecounty or county groups?
...
imputations_via_crossgeo <- function(edf, locf_for_up_to) {
  ...
  facility_props <-
    ...
    filter(
      ...
      max_unique_time_value - locf_for_up_to - 7L*25L <= time_value,
      # # FIXME probably need this longer averaging but to be more reactive to
      # # drop-out?... and deal with NAs..
      time_value <= max_unique_time_value - locf_for_up_to
    ) %>%
    # TODO consider filtering to same wday
    #
    # TODO consider by county or county groups?
    summarize(.by = geo_value, value = sum(value, na.rm = TRUE)) %>%
...
# imputations_via_last_obs_fixup <- function(partial_archive, locf_for_up_to) {
#   latest_edf <- partial_archive %>% epix_as_of(.$versions_end)
#   unique_time_values <- unique(latest_edf$time_value)
#   geo_latencies <-
#     latest_edf %>%
#     summarize(.by = geo_value, max_time_value = max(time_value)) %>%
#     mutate(min_version_lag = as.integer(partial_archive$versions_end - max_time_value))
#   # TODO consider an average of multiple lags rather than just a single
#   # observation, which might be noisy
#   analogues <- partial_archive %>%
#     epix_slide(
#       ref_time_values = sort(unique(partial_archive$DT$version)) %>%
#         `[`(as.POSIXlt(.)$wday == as.POSIXlt(tail(., 1L))$wday),
#       before = max(geo_latencies$min_version_lag),
#       function(edf, gk, version) {
#         geo_latencies %>%
#           transmute(geo_value, time_value = .env$version - min_version_lag) %>%
#           left_join(edf, by = c("geo_value", "time_value")) %>%
#           select(geo_value, last_obs_analogue = value)
#       }, names_sep = NULL
#     )
#   # browser()
#   stop('fixme alignment time_value vs. version, plus the target specification')
#   # latest_edf %>%
#   #   left_join(analogues, by = c("geo_value", "time_value")) %>%
#   latest_edf %>%
#     left_join(analogues, by = c("geo_value", "time_value")) %>%
#     slice_max(time_value, by = geo_value) %>%
#     select(geo_value, time_value, value, last_obs_analogue)
# }
...
# facilities_imputations5 <-
#   archive %>%
#   epix_slide(
#     ref_time_values = prototyping_versions,
#     all_versions = TRUE,
#     before = 365000L,
#     function(partial_archive, gk, version) {
#       imputations_via_last_obs_fixup(partial_archive, facility_locf_for_up_to)
#     },
#     as_list_col = TRUE
#   ) %>%
#   rename(version = time_value) %>%
...
inner_join(imps_bound, facilities_truth_edf, by = c("geo_value", "state", "fips_code", "time_value"), suffix = c("_imputation", "_truth")) %>%
  summarize(.by = model,
            ...
            MAEnormlzd = mean(abs(value_imputation - value_truth), na.rm = TRUE)/mean(value_truth)*100,
            P80AE = quantile(abs(value_imputation - value_truth), 0.8, na.rm = TRUE))

facility_means <- facilities_truth_edf %>%
  summarize(.by = geo_value, facility_mean = mean(value, na.rm = TRUE)) %>%
  mutate(facility_mean_cat = cut(facility_mean, unique(quantile(facility_mean, (0:4)/4)), include.lowest = TRUE))

inner_join(imps_bound, facilities_truth_edf, by = c("geo_value", "state", "fips_code", "time_value"), suffix = c("_imputation", "_truth")) %>%
  left_join(facility_means, by = "geo_value") %>%
  summarize(.by = c(facility_mean_cat, model),
            N = n(),
            MAE = mean(abs(value_imputation - value_truth), na.rm = TRUE),
            MAEnormlzd = mean(abs(value_imputation - value_truth), na.rm = TRUE)/mean(value_truth)*100,
            P80AE = quantile(abs(value_imputation - value_truth), 0.8, na.rm = TRUE)) %>%
  arrange(facility_mean_cat, model)
...
TODO scores by facility mean

TODO weighted combination by proximity and volume



County-level aggregation analysis
```{r}
counties_truth <- facilities_truth_edf %>%
  summarize(.by = c(fips_code, time_value), value = sum(value))

# No need to deal with NAs:
counties_truth %>%
  filter(is.na(value)) %>%
  with(range(time_value))
range(prototyping_versions)

county_imps_list <- lapply(imps_list, function(facility_imp) {
  epix_slide(
    archive,
    ref_time_values = prototyping_versions,
    before = 365000L,
    function(edf, gk, version) {
      edf %>%
        add_imputations(facility_imp) %>%
        summarize(.by = c(fips_code, time_value), value = sum(value)) %>%
        # only look at county x times that had some imputation going on
        inner_join(distinct(facility_imp, fips_code, time_value),
                   by = c("fips_code", "time_value"))
    }, as_list_col = TRUE
  ) %>%
    rename(version = time_value) %>%
    unnest(slide_value)
...
county_imps_bound <-
  ...
  bind_rows(.id = "model") %>%
  filter(!is.na(value)) %>%
  group_by(version, fips_code, time_value) %>%
  filter(n() == length(county_imps_list)) %>%
  ungroup()

county_means <- counties_truth %>%
  summarize(.by = fips_code, county_mean = mean(value, na.rm = TRUE)) %>%
  mutate(county_mean_cat = cut(county_mean, unique(quantile(county_mean, (0:4)/4)), include.lowest = TRUE))

inner_join(county_imps_bound, counties_truth, by = c("fips_code", "time_value"), suffix = c("_imputation", "_truth")) %>%
  summarize(.by = model,
            MAE = mean(abs(value_imputation - value_truth), na.rm = TRUE),
            MAEnormlzd = mean(abs(value_imputation - value_truth), na.rm = TRUE)/mean(value_truth)*100,
            P80AE = quantile(abs(value_imputation - value_truth), 0.8, na.rm = TRUE))
...
inner_join(county_imps_bound, counties_truth, by = c("fips_code", "time_value"), suffix = c("_imputation", "_truth")) %>%
  ...
  summarize(.by = c(county_mean_cat, model),
            N = n(),
            MAE = mean(abs(value_imputation - value_truth), na.rm = TRUE),
            MAEnormlzd = mean(abs(value_imputation - value_truth), na.rm = TRUE)/mean(value_truth)*100,
            P80AE = quantile(abs(value_imputation - value_truth), 0.8, na.rm = TRUE)) %>%
  arrange(county_mean_cat, model)
